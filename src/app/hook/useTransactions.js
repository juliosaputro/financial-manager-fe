...
const API_BASE_URL = `${process.env.PORT}/api`;
...
const useTransactions = (initialType = null) => {
  ...
  const fetchTransactions = useCallback(async (newFilters = {}, options = {}) => {
    setLoading(true);
    setError(null);
    try {
      ...
      if (currentFilters.year && !currentFilters.startDate) params.append('year', currentFilters.year);
      if (currentFilters.month && !currentFilters.startDate) params.append('month', currentFilters.month);
      ...
      console.error(`Error fetching ${initialType || 'all'} transactions:`, err);
    } finally {
      setLoading(false);
    }
  ...
  const addTransaction = async (newTransactionData) => {
    ...
    const payload = { ...newTransactionData, type: initialType };
  ...
  const updateTransaction = async (transactionId, updatedData) => {
    ...
    const payload = { ...updatedData, type: initialType };
  ...
  const deleteTransaction = async (transactionId) => {
    await fetch(`${API_BASE_URL}/transactions/${transactionId}`, { method: 'DELETE' });
    fetchTransactions(filters, { page: pagination.currentPage });
  };
